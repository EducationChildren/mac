 ...
 */
- (void)snapshotFromLocalVideoCompletion:(void(^)(UIImage * __nullable image))result API_UNAVAILABLE(macos);
...
/**
 ...
 @param scale 放大倍数
 */
- (void)changeLensPosition:(float)scale API_UNAVAILABLE(macos);
...
/**
 ...
 @param mode  对焦模式
 */
- (void)setFocusMode:(NIMNetCallFocusMode)mode API_UNAVAILABLE(macos);
 ...
 */
- (void)setContrastFilterIntensity:(float)value API_UNAVAILABLE(macos);
 ...
 */
- (void)setSmoothFilterIntensity:(float)value API_UNAVAILABLE(macos);
